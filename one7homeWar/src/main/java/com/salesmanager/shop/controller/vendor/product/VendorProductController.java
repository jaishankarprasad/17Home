...
public class VendorProductController extends AbstractController {
	...
	//Based On  ProductId Fetch All The Vendor That are Related on That Product
	@RequestMapping(value={"/FindVendorsByProductId/{productId}"},method = {RequestMethod.GET})
	@ResponseBody
	public VendorsList getVendorByProductId(@PathVariable Long productId,HttpServletRequest request) {
		LOGGER.debug("Entered getProductVendors");
		VendorsList vendorsList=new VendorsList();
		List<VendorResponse>vendorDetailsforProduct=new ArrayList<VendorResponse>();
		try {
			List<VendorProduct> dbVendorProductList =vendorProductService.findVendorByProductId(productId);
			if(dbVendorProductList == null || dbVendorProductList.isEmpty()){
				vendorsList.setStatus("False");
				vendorsList.setMessage("No Vendor List Available");
				return vendorsList;
			}
			LOGGER.debug("dbVendorProductList size=="+dbVendorProductList.size());
			for(VendorProduct vendorProduct : dbVendorProductList){
				...
				if(vendor != null) {
					VendorPopulator vendorPopulator = new VendorPopulator();
					vendorPopulator.populate(vendor, vendorResponse, null, null);
					vendorDetailsforProduct.add(vendorResponse);
				}
			}
			vendorsList.setVendorsDataForProduct(vendorDetailsforProduct);
		}catch(Exception e){
			LOGGER.error("error occured while retrieving vendors based on product id ="+productId+"---"+e.getMessage());
		}
		vendorsList.setStatus("True");
		vendorsList.setMessage("Vendor List");
		LOGGER.error("getProductVendors");
		return vendorsList;
     }
