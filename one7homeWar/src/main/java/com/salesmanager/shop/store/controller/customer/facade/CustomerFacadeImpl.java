...
/**
 * Customer Facade work as an abstraction layer between Controller and Service
 * layer. It work as an entry point to service layer.
 * 
...
public class CustomerFacadeImpl implements CustomerFacade {
	...
	/**
	 * Method used to fetch customer based on the username and storecode. Customer
	 * username is unique to each store.
	 *
	 * @param userName
	 * @param store
	...
	/*
	 * @Override public CustomerEntity getCustomerDataByCustomer( final Customer
	 * customer, final MerchantStore store, final Language language ) throws
	 * Exception { LOG.info( "Fetching customer with userName" +customer.getNick());
	 * if(customer !=null){ LOG.info(
	 * "Found customer, converting to CustomerEntity"); try{ CustomerEntityPopulator
	 * customerEntityPopulator=new CustomerEntityPopulator(); return
	 * customerEntityPopulator.populate( customer, store, language ); //store,
	 * language } catch(ConversionException ex){ LOG.error(
	 * "Error while converting Customer to CustomerEntity", ex ); throw new
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.salesmanager.web.shop.controller.customer.facade#mergeCart(final
	 * Customer customerModel, final String sessionShoppingCartId ,final
	 * MerchantStore store,final Language language)
	 */
	@Override
			...
			if (StringUtils.isNotBlank(sessionShoppingCartId)) {
				...
				if (sessionShoppingCart != null) {
					if (customerCart == null) {
						if (sessionShoppingCart.getCustomerId() == null) {// saved shopping cart does not belong to a
																			// customer
	...
	@Override
	public Customer getCustomerByPhone(String phone, MerchantStore store) throws Exception {
		return customerService.getByPhone(phone, store.getId());
	}
	/**
	 * <p>
	 * Method to check if given user exists for given username under given store.
	 * System treat username as unique for a given store, customer is not allowed to
	 * use same username twice for a given store, however it can be used for
	 * different stores.
	 * </p>
	 * 
	...
	public boolean checkIfUserExists(final String userName, final MerchantStore store) throws Exception {
		if (StringUtils.isNotBlank(userName) && store != null) {
			...
			if (customer != null) {
				LOGGER.info("Customer with userName {} already exists for store {} ", userName, store.getStorename());
				return true;
			}
	...
	@Override
	public boolean checkIfMobileExists(String phone, MerchantStore store) throws Exception {
		if (StringUtils.isNotBlank(phone) && store != null) {
			Customer customer = customerService.getByPhone(phone, store.getId());
			if (customer != null) {
				LOGGER.info("Customer with phone {} already exists for store {} ", phone, store.getStorename());
				return true;
			}

			LOGGER.info("No customer found with phone {} for store {} ", phone, store.getStorename());
			return false;

		}
		LOGGER.info("Either phone is empty or we have not found any value for store");
		return false;
	}
	...
	@Override
	public Customer getCustomerModel(final PersistableCustomer customer, final MerchantStore merchantStore,
			Language language) throws Exception {
	...
	private CustomerEntity customerEntityPoulator(final Customer customerModel, final MerchantStore merchantStore) {
		...
		try {
			CustomerEntity customerEntity = customerPopulator.populate(customerModel, merchantStore,
					merchantStore.getDefaultLanguage());
			if (customerEntity != null) {
				customerEntity.setId(customerModel.getId());
				LOGGER.info("Retunring populated instance of customer entity");
				return customerEntity;
			}
			LOGGER.warn("Seems some issue with customerEntity populator..retunring null instance of customerEntity ");
			return null;

		} catch (ConversionException e) {
			LOGGER.error("Error while converting customer model to customer entity ", e);
	...
	public void authenticate(Customer customer, String userName, String password) throws Exception {
		...
		GrantedAuthority role = new SimpleGrantedAuthority(ROLE_PREFIX + Constants.PERMISSION_CUSTOMER_AUTHENTICATED);// required
																														// to
																														// login
	...
	@Override
	public void updateAddress(Customer customerModel, MerchantStore merchantStore, Address address,
			final Language language) throws Exception {
		...
		if (customerModel == null) {
			LOGGER.error("Customer with ID {} does not exists..", customerModel.getEmailAddress());
			// throw new CustomerNotFoundException( "customer with given id does not exists"
			// );
			throw new Exception("Customer with given id does not exists");
		}
	...
	private Customer updateFirstDeliveryAddressOne(Customer customerModel, MerchantStore merchantStore, Address address,
		...
		// PersistableCustomerSecondaryShippingAddressPopulator
		// shippingAddressPopulator=new
		// PersistableCustomerSecondaryShippingAddressPopulator();
	...
	private Customer updateSecondaryDeliveryAddress(Customer customerModel, MerchantStore merchantStore,
			Address address, Country country) throws ConversionException {
		...
		customerModel = shippingAddress2Populator.populate(address, customerModel, merchantStore,
				merchantStore.getDefaultLanguage());
	...
	public Customer populateCustomerModel(Customer customerModel, PersistableCustomer customer,
		...
		// customerModel.setPassword(passwordEncoder.encodePassword(customer.getPassword(),
		// null));
		// setCustomerModelDefaultProperties(customerModel, merchantStore);
		// }
