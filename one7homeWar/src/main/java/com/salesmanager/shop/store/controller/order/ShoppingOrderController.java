...
/*	    if(customer!=null) {
	...
	private Order commitOrder(ShopOrder order, HttpServletRequest request, Locale locale) throws Exception, ServiceException {
			...
			LOGGER.debug("before remvoing Cart object, cartCode "+cartCode);
	...
	@SuppressWarnings("unchecked")
	@RequestMapping(value="/commitCCAvenueOrder")
	@ResponseBody
	public CCAvenueInfoResponse commitCCAvenueOrder(@RequestBody OrderRequest orderRequest,
			HttpServletRequest request, Locale locale) throws Exception {
		 //ReadableOrder readableOrder = new ReadableOrder();
		 Integer preferedShippingAddress = Integer.valueOf(orderRequest.getPreferedShippingAddress());
		 String cartCode = orderRequest.getCartCode();
		 LOGGER.debug("userId "+orderRequest.getUserId()	+" preferedShippingAddress : "+preferedShippingAddress);
		 ...
		 LOGGER.debug("Cart Items "+cart.getLineItems());
		 ...
		 LOGGER.debug("Order Id "+orderId+" amount is "+amount);
		 return ccAvenuPaymenteRequestData(amount, orderId,customer);
	}
	
	/**
	 * Create the Order and Complete it immediatly, since this order type is COD.
	 * @param orderRequest
	 * @param request
	 * @param locale
	 * @return
	 ...
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping(value="/completeCOD_Order")
	@ResponseBody
	public ReadableOrder completeCOD_Order(@RequestBody OrderRequest orderRequest,
			HttpServletRequest request, Locale locale) throws Exception {
			 //ReadableOrder readableOrder = new ReadableOrder();
			 Integer preferedShippingAddress = Integer.valueOf(orderRequest.getPreferedShippingAddress());
			 String cartCode = orderRequest.getCartCode();
		     ...
		     shopOrder.setShippingCharges(cart.getShippingCharges());
		     shopOrder.setPaymentType(PaymentType.CashOnDelevery);
		     shopOrder.setPreferedShippingAddress(preferedShippingAddress);
		     shopOrder.setIpAddress(request.getLocalAddr());
	 	     OrderTotalSummary totalSummary = super.getSessionAttribute(Constants.ORDER_SUMMARY, request);
			 if(totalSummary==null) {
			...
			orderHistory.setOrder(modelOrder);
			orderHistory.setStatus(OrderStatus.ORDERED);
			orderHistory.setDateAdded(new Date());
			orderService.addOrderStatusHistory(modelOrder, orderHistory);
			modelOrder.setStatus(OrderStatus.ORDERED);
			orderService.saveOrUpdate(modelOrder);
			Map<String,String> transactionCompleteData = new HashMap();
			transactionCompleteData.put("orderDetails ", "COD Order Created");
			// Now we have to send an email to customer / vendor and admin here
			paymentService.createTransactionObject(modelOrder.getTotal(),modelOrder,transactionCompleteData,PaymentType.CashOnDelevery);
			Customer modelCustomer = getSessionAttribute(Constants.CUSTOMER, request );
			try {
				...
				// testing purpose sending to ram
			...
			return orderFacade.getReadableOrderByOrder(modelOrder, store, language);
	}
	
	private CCAvenueInfoResponse ccAvenuPaymenteRequestData(String amt,String orderId,Customer customer) throws UnsupportedEncodingException{
		CCAvenueInfoResponse ccavResponse = new CCAvenueInfoResponse();
		...
		String ccAvenuePaymentGatewayURL = cCAvenuePaymentConfiguration.getCcAvenuePaymentGatewayURL();
...
*/		
		...
		ccavResponse.setEncRequest(encRequest);
		ccavResponse.setAccessCode(accessCode);
		ccavResponse.setPaymentGatewayUrl(ccAvenuePaymentGatewayURL);
		LOGGER.debug("response "+ccavResponse);
		return ccavResponse;
	}
	
	@RequestMapping(value="/cancelOrder", method = RequestMethod.POST)
	@ResponseBody
	public String cancelOrder(@RequestBody OrderRequest orderRequest,HttpServletRequest request, Locale locale) throws Exception {
		Long orderId = Long.valueOf(orderRequest.getOrderId());
		...
		// send Email to customer saying, order got canceled.
		// Send email to Vendor and Admin and inform the same.
		return "Order canceled "+orderRequest.getOrderId();
	}

	@RequestMapping(value="/completeCCAvenueOrder", method = RequestMethod.POST)
	@ResponseBody
	public ReadableOrder completeCCAvenueApprovedOrder(@RequestBody OrderRequest orderRequest,
							HttpServletRequest request) throws Exception {
		String encResponse = orderRequest.getEncData();
		LOGGER.info("encRes:"+ encResponse);
		Map<String,String> completeResponseData = decryptRequiredResponse(encResponse);
		Long orderId = Long.valueOf(completeResponseData.get("order_id"));
		...
		if(SUCCESS.equals(completeResponseData.get(ORDER_STATUS))){
		...
		// Now we have to send an email to customer / vendor and admin here
		paymentService.createTransactionObject(order.getTotal(),order,completeResponseData,PaymentType.CCAvenue);
		Long customerId = order.getCustomerId();
		Customer modelCustomer = null;
		if(customerId != null){
			modelCustomer = customerService.getById(customerId);
    		if(modelCustomer!=null) {
    			request.setAttribute(Constants.CUSTOMER, modelCustomer);
				request.getSession().setAttribute(Constants.CUSTOMER, modelCustomer);					
    		}
		}
		Language language = (Language)request.getAttribute("LANGUAGE");
		MerchantStore store = (MerchantStore)request.getAttribute(Constants.MERCHANT_STORE);
		Locale locale = LocaleUtils.getLocale(store);
		try {
			//send order confirmation email to customer
			if(emailTemplatesUtils!=null) {
				emailTemplatesUtils.sendOrderEmail(modelCustomer.getEmailAddress(), modelCustomer, order, locale, language, store, request.getContextPath());
				//send order confirmation email to merchant
				emailTemplatesUtils.sendOrderEmail(store.getStoreEmailAddress(), modelCustomer, order, locale, language, store, request.getContextPath());
				// testing purpose sending to ram
				emailTemplatesUtils.sendOrderEmail("rambabu0006@gmail.com", modelCustomer, order, locale, language, store, request.getContextPath());
			}
		} catch(Exception e) {
			LOGGER.error("Error while post processing order",e);
		}
		return orderFacade.getReadableOrderByOrder(order, store, language);
	}
	/*@RequestMapping(value="/completeOrder/{orderId}", method = RequestMethod.POST)
	@ResponseBody
	public ReadableOrder completeOrder(@PathVariable Long orderId,HttpServletRequest request, Locale locale) throws Exception {
		Order order = orderService.getById(orderId);
		OrderStatusHistory orderHistory = new OrderStatusHistory();
		orderHistory.setOrder(order);
		...
		Map<String,String> completeResponseData = decryptRequiredResponse(request, reqParams, data);
		OrderStatus status = null;
		...
		// Now we have to send an email to customer / vendor and admin here
		paymentService.createTransactionObject(order.getTotal(),order,completeResponseData);
		MerchantStore store = (MerchantStore)request.getAttribute(Constants.MERCHANT_STORE);
		Language language = (Language)request.getAttribute("LANGUAGE");
		return orderFacade.getReadableOrderByOrder(order, store, language);
	}*/
	@RequestMapping(value="/orderFailure", method = RequestMethod.POST)
	@ResponseBody
	public String orderFailure(@RequestBody OrderRequest orderRequest,HttpServletRequest request, Locale locale) throws Exception {
		Long orderId = Long.valueOf(orderRequest.getOrderId());
		String encResponse = orderRequest.getEncData();
		Order order = orderService.getById(orderId);
		OrderStatusHistory orderHistory = new OrderStatusHistory();
		...
		// Now we have to send an email to customer / vendor and admin here
		paymentService.createTransactionObject(order.getTotal(),order,completeResponseData,PaymentType.CCAvenue);
		return "Order failure "+orderId;
	}


	private Map<String,String> decryptRequiredResponse(String encResp){
		String workingKey = cCAvenuePaymentConfiguration.getWorkingKey();
        AesCryptUtil aesUtil=new AesCryptUtil(workingKey);
        String decResp = aesUtil.decrypt(encResp);
        StringTokenizer tokenizer = new StringTokenizer(decResp, "&");
        Map<String,String> completeResponseData=new HashMap<String,String>();
        ...
        while (tokenizer.hasMoreTokens()) {
                ...
                if(pair!=null) {
                        ...
                        if(strTok.hasMoreTokens()) {
                                ...
                                completeResponseData.put(pname, pvalue);
                        }
                }
        }
        LOGGER.debug(" decryptRequiredResponse completeResponseData "+completeResponseData);
