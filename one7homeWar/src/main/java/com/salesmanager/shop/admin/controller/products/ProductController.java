...
*/		
	...
	@ResponseBody
	public ProductDetails getProduct(@PathVariable String prodId,@RequestParam(defaultValue = "true") Boolean isRecomendedProductsNeeded) throws Exception {
		...
		if(dbProduct != null) {
			...
			Set<Category> dbCategories= dbProduct.getCategories();

			for(Category category:dbCategories){
				if(category != null)
					productDetails.setCategoryCode(category.getCode().replaceAll(" ", "_"));
				if(category.getParent() != null)
					productDetails.setParentCategoryCode(category.getParent().getCode().replaceAll(" ", "_"));	
			}
			...
			Double avgReview =reviewController.getAvgReview(reviews);
			Long totalRatingCount = reviewController.getTotalRatingCount(reviews);
			if(avgReview!=null) {
				productDetails.setAvgReview(avgReview);
			}
			if(totalRatingCount!=null) {
				productDetails.setTotalratingCount(totalRatingCount);
			}
			...
			if(productPrice.getProductPriceSpecialAmount() != null) {
				if(productPrice.getProductPriceSpecialStartDate() != null && productPrice.getProductPriceSpecialEndDate() != null) {
					if(productPrice.getProductPriceSpecialEndDate().compareTo(productPrice.getProductPriceSpecialStartDate()) >1){
		...
		if(isRecomendedProductsNeeded) {
			productDetails.setRelatedProductList(getRelatedProductList(productDetails));
			productDetails.setRecomendedProductList(getRecomendedProductList(productDetails, dbProduct));
		}
		LOGGER.debug("Ended getProduct");
		return productDetails;		
	}
	/**
	 * Recommended Products should show list of products from the same category apart from selected sub-category (4 to 5 products)
	 *  Recommended Products should be also displayed in the Category page with the same criteria as mentioned above
	 * @throws Exception 
	 */
    @Async
	public List<ProductDetails> getRecomendedProductList(ProductDetails productDetails,Product dbProduct) throws Exception{
		int totalRecomendedQtr = 10;
		List<ProductDetails> recomendedProductList = new ArrayList();
		String selectedCategoryCode = productDetails.getCategoryCode().replaceAll("_", " ");
		
		//String parentCategoryCode = productDetails.getParentCategoryCode();
		Set<Category> dbCategories= dbProduct.getCategories();
		System.out.println("dbCategories size  =="+dbCategories.size());
		Category parentCategory = null;
		for(Category category:dbCategories){
			if(category.getParent() != null)
				parentCategory = category.getParent();	
		}
		List<Category> child = parentCategory.getCategories();
		List<Long> sliblingCategoryIds = child.stream()
			.filter(e -> !selectedCategoryCode.equals(e.getCode()))
			.map(e ->  e.getId()).collect(Collectors.toList());
		System.out.println("sliblingCategoryIds "+sliblingCategoryIds);
		/*List<Product>  productsBelongToSliblingsCategories = productService.getProducts(sliblingCategoryIds);
		int sliblingProductSize = productsBelongToSliblingsCategories.size();
		List<Product> randomProduts;
		int numberOfRecomendedProduts = 8;
		Set<Product> randomProducts = new HashSet();
		for(int i=0;i<sliblingProductSize;i++) {
			numberOfRecomendedProduts.get(8);
			randomProducts.add();
		}*/
		
		List<BigInteger> productIds = productService.getProductIdByCategoryId(sliblingCategoryIds);
		System.out.println("productIds "+productIds);
		int totalProductSize = productIds.size();
		if(totalProductSize == 0) {
			return recomendedProductList;
		}
		System.out.println("productIds 0 "+productIds.get(0));
		Set<Long> limitedRecomendedProductSet = new HashSet<Long>();
		if(totalProductSize > totalRecomendedQtr) {	
			for(int i=0;i<totalRecomendedQtr;i++) {
				int randomNum = getRandom(totalProductSize); // we getting the random products, if reccomended products size  is more than required.
				Long recProduId = ((BigInteger)productIds.get(randomNum)).longValue();
				System.out.println( " recProduId "+recProduId	);
				limitedRecomendedProductSet.add(recProduId);
			}
			System.out.println("limitedRecomendedProductSet  "+limitedRecomendedProductSet);

			for(Long productId : limitedRecomendedProductSet) {
				//Product eachProd = productService.getByProductId(productId);
				ProductDetails prod = getProduct(productId.toString(),false);
				recomendedProductList.add(prod);
			}
		}else {
	...
	/**
	 *  Related Products should be from the same sub-category but sort by price (only top 3)
	 *  Compare With Related Products should have
	.	1st product is the default selected product
	.	Other will be in High, Medium & Low related products
	 * @throws Exception 
	 */
    @Async
	public List<ProductDetails> getRelatedProductList(ProductDetails productDetails) throws Exception{
		List<ProductDetails> relatedProductList = new ArrayList();
		//relatedProductList.add(productDetails);
		String categoryCode = productDetails.getCategoryCode().replaceAll("_", " ");
		Long selectedProductId = productDetails.getProductId();
		long limit = 6;	
		List<BigInteger> productIds = productService.getMaxPriceProductsByLimitingTo(categoryCode, limit);
		//System.out.println(" productIds "+productIds);
		for(BigInteger prodId : productIds) {
			if(selectedProductId.equals(prodId.longValue())) { // if productId is selected product, then we should ignore
				continue;
			}
			ProductDetails prod = getProduct(prodId.toString(),false);
			relatedProductList.add(prod);
		}
		//System.out.println(" relatedProductList "+relatedProductList);
		return relatedProductList;	
	}
	
	/*private List<ProductDetails> getRelatedProductList(ProductDetails productDetails,Product dbProduct) throws ServiceException{
		List<ProductDetails> relatedProductList;
		Set<Category> dbCategories= dbProduct.getCategories();
		Category parentCategory = null;
		for(Category category:dbCategories){
			if(category.getParent() != null)
		...
		List<Category> childCategories = categoryService.listByParent(parentCategory);
		childCategories.stream()
						.filter(new Predicate<Category>() {
							@Override
							public boolean test(Category t) {
								// TODO Auto-generated method stub
								return t.getCode() != productDetails.getCategoryCode();
							}
						})
						.map(new Function<Category, String>() {
							@Override
							public String apply(Category t) {
								// TODO Auto-generated method stub
								return t.getCode();
							}
						}).collect(Collectors.toList());
		//productService.getMaxPriceProductsByLimitingTo(categoryCode, limit);
		return null;
	}*/
