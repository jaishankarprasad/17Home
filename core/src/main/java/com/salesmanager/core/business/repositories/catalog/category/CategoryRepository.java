...
public interface CategoryRepository extends JpaRepository<Category, Long>, CategoryRepositoryCustom {
	...
	@Query("select distinct c from Category c left join fetch c.descriptions cd join fetch cd.language cdl join fetch c.merchantStore cm where cm.id=?1 order by  c.sortOrder desc")
...
/*	@Query("select distinct c  from Category c inner join fetch c.services where c.code=?1")
	Category getServiceByCategoryCode(String code);
	*/
	@Query(value = "SELECT DISTINCT S.ID FROM CATEGORY C JOIN SERVICES S ON C.CATEGORY_ID=S.CATEGORY_ID AND C.CODE in (?1)",nativeQuery=true)
    List<BigInteger> getServicesByCategoryCode(String categoryCode);
