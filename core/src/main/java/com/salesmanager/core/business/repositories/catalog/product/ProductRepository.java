...
public interface ProductRepository extends JpaRepository<Product, Long>, ProductRepositoryCustom {
    ...
    @Query(value = "SELECT P.PRODUCT_ID FROM PRODUCT_PRICE PRICE "
    		+ " INNER JOIN PRODUCT_AVAILABILITY AVAIL ON  PRICE.PRODUCT_AVAIL_ID = AVAIL.PRODUCT_AVAIL_ID"
    		+ " INNER JOIN  PRODUCT P ON   AVAIL.PRODUCT_AVAIL_ID= P.PRODUCT_ID WHERE PRODUCT_PRICE_ID IN"
    		+ " (SELECT PRODUCT_PRICE_ID FROM PRODUCT_PRICE PRICE INNER JOIN PRODUCT_AVAILABILITY AVAIL "
    		+ " WHERE PRICE.PRODUCT_AVAIL_ID = AVAIL.PRODUCT_AVAIL_ID AND  PRODUCT_ID IN (SELECT PRODUCT_ID"
    		+ " FROM PRODUCT_CATEGORY WHERE CATEGORY_ID = (SELECT CATEGORY_ID FROM CATEGORY WHERE CODE = ?1))) "
    		+ "ORDER BY PRODUCT_PRICE_AMOUNT DESC LIMIT ?2",nativeQuery=true)
    List<BigInteger> getMaxPriceProductsByLimitingTo(String categoryCode,Long limit);
    
    @Query(value = "SELECT DISTINCT PRODUCT_ID FROM PRODUCT_CATEGORY WHERE CATEGORY_ID in (?1)",nativeQuery=true)
    List<BigInteger> getProductIdByCategoryId(List<Long> categoryIds);
